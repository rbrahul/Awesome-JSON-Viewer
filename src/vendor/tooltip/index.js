class Tooltip {
    ...
    getMatchedClasses(classList, expectedClasses=[]) {
        return expectedClasses.filter(expectedClass => classList.contains(expectedClass));
    }
    ...
    getOffset(target,direction=this.direction) {
        ...
        let offsetLeft = rect.left + window.scrollX + rect.width + this.distance;
        ...
        switch(direction) {
            ...
            case 'right':
                    ...
                    offsetLeft = rect.left + window.scrollX + rect.width + this.distance;
                    break;
            case 'left':
                    offsetTop = rect.top + window.scrollY + rect.height / 2;
                    ...
                    break;
            case 'bottom':
                    offsetTop = rect.top + window.scrollY + rect.height + this.distance;
    ...
    showToolTip(event){
        ...
        const {top:offsetTop,left:offsetLeft} = this.getOffset(event.target, direction);
        ...
        if('tooltip' in dataset) {
            ...
            const matchedClasses = this.getMatchedClasses(tooltipNode.classList, this._allowedDirection.map(className =>`dir-${className}`));

            if (!!matchedClasses.length) {
                matchedClasses.forEach(className => tooltipNode.classList.remove(className));
            }
        ...
        if ('tooltipSize' in dataset) {
            if (tooltipNode?.classList && !tooltipNode.classList.contains(dataset.tooltipSize)) {
    ...
    destroy() {
        ...
        const tooltipMsgElement = document.querySelector(`#${this.tooltipMessageId}`);
